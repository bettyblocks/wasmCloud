//! Per-workload stream broker for components that need separate invocations to
//! rendezvous around a long-lived client connection.
...
const PLUGIN_STREAM_BROKER_ID: &str = "betty-blocks-stream-broker";
...
impl<'a> bindings::betty_blocks::stream_broker::broker::Host for ActiveCtx<'a> {
...
impl HostPlugin for StreamBroker {
    ...
    fn world(&self) -> WitWorld {
        WitWorld {
            imports: HashSet::from([WitInterface::from(
                "betty-blocks:stream-broker/broker@0.1.0",
            )]),
            ..Default::default()
        }
    }
    ...
    async fn on_workload_item_bind<'a>(
        ...
        let Some(_interface) = interfaces
            .iter()
            .find(|i| i.namespace == "betty-blocks" && i.package == "stream-broker")
        else {
            return Ok(());
        };

        bindings::betty_blocks::stream_broker::broker::add_to_linker::<_, SharedCtx>(
            item.linker(),
            extract_active_ctx,
        )?;
