...
//
// Plugin lookups use `try_get_plugin` — upstream's `get_plugin` returns `Arc<T>`
// and panics if absent; `try_get_plugin` preserves the graceful fallback.
impl<'a> bindings::betty_blocks::cancellation_broker::broker::Host for ActiveCtx<'a> {
    async fn set_cancel(&mut self, plan_id: String, cancelled: bool) -> wasmtime::Result<()> {
        let store_handle = {
            let plugin =
                match self.try_get_plugin::<CancellationBroker>(PLUGIN_CANCELLATION_BROKER_ID) {
                    Ok(plugin) => plugin,
                    Err(e) => {
                        wasmtime::bail!("CancellationBroker plugin not found in context: {e}");
                    }
                };
            plugin.inner.clone()
        };
    ...
    for SharedCtx
{
    async fn wait_cancel(
        ...
        let store_handle = accessor.with(|mut access| {
            let ctx = access.get();
            let plugin = ctx
                .try_get_plugin::<CancellationBroker>(PLUGIN_CANCELLATION_BROKER_ID)
                .map_err(|e| {
                    wasmtime::Error::msg(format!(
                        "CancellationBroker plugin not found in context: {e}"
                    ))
                })?;
            Ok::<_, wasmtime::Error>(plugin.inner.clone())
        })?;
...
impl HostPlugin for CancellationBroker {
    ...
    async fn on_workload_item_bind<'a>(
        ...
        let has_broker = interfaces
            .iter()
            .any(|i| i.namespace == "betty-blocks" && i.package == "cancellation-broker");
