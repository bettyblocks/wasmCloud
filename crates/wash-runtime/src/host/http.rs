...
#[instrument(skip_all, fields(
    // BettyBlocks: trace_id span field for structured logging (retained in upstream merge).
...
async fn handle_http_request<T: Router>(
    ...
    let workload_id = match handler.route_incoming_request(&req) {
        ...
        Err(e) => {
            ...
            let resp = error_response(e.status(), &trace_id);
                ...
                .await
            {
                Ok(mut resp) => {
                    if let Ok(trace_header) = hyper::header::HeaderValue::from_str(&trace_id) {
                        resp.headers_mut().insert(TRACE_ID_HEADER, trace_header);
                    }
                    resp
                }
                Err(e) => {
