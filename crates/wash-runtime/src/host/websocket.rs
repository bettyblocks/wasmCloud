...
//! `betty-blocks:websockets/handler@0.1`.
//!
//! Flow:
...
/// exports `betty-blocks:websockets/handler@0.1`.
///
...
async fn run_ws_session(
    ...
    let peer_closed = PeerClosed::default();
    let producer = WsReadProducer::new(ws_read, peer_closed.clone());
    let (write_tx, write_rx) = mpsc::channel(WS_WRITE_QUEUE_CAPACITY);
    spawn_ws_write_forwarder(ws_write, write_rx, close_tx, peer_closed);
    ...
    let result = store
        .run_concurrent(async move |accessor| {
            ...
            let call = instance
                .betty_blocks_websockets_handler()
...
fn spawn_ws_write_forwarder(
    ...
    tokio::spawn(async move {
        while !peer_closed.is_closed() {
            let msg = tokio::select! {
                biased;

                _ = peer_closed.notify.notified() => break,
                msg = write_rx.next() => msg,
            };
