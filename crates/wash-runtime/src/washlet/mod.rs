...
impl ClusterHost {
    ...
    pub async fn start(
        ...
        let compiled_cleanup_interval = self.compiled_cleanup_interval;
        let compiled_cleanup_grace = self.compiled_cleanup_grace;
            ...
            async move {
                ...
                loop {
                    tokio::select! {
                        ...
                        // host, and — per the `select!` note above — so it never delays the
                        // heartbeat either.
                        //
                        // Commands naming one workload are ordered by the host's reservation on
                        // that id, not by this loop: a start claims the id before it fetches
                        // anything, and a stop that finds the claim hands the teardown back to
                        // the start holding it.
                                ...
                                match response {
                                    Ok(resp_bytes) => {
                                        if let Some(reply_to) = msg.reply {
                                            ...
                                            // backpressure) — a hang here would otherwise leave
                                            // this command's task stuck forever even though its
                                            // actual work already finished successfully.
...
async fn handle_command(
    ...
    match command {
        ...
        "workload.start" => {
            ...
            let res = workload_start(host, req, config, data_nats_client, starts).await?;
            to_api(&res)
        }
        "workload.stop" => {
...
fn component_from_wire(
    ...
    crate::types::Component {
        ...
        source: crate::types::Source::Compile(loaded.bytes),
    ...
    let prepared = async {
        let (components, host_interfaces) = if let Some(wit_world) = wit_world {
            ...
            for component in &wit_world.components {
                // A precompiled component skips the OCI pull entirely: its bytes
                // never come to the host, only a `.cwasm` fetched (or already
                // cached) under the compiled-cache dir and mapped file-backed.
                if !component.precompiled_url.is_empty() {
                    ...
                    let Some(dir) = config.compiled_cache_dir.as_deref() else {
                        return Err(format!(
                    ...
                    let source = match download_cwasm(
