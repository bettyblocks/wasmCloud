...
// TestHostReconcilerRegistryActionsByReadyState pins down the user-facing
...
func TestHostReconcilerRegistryActionsByReadyState(t *testing.T) {
	cases := []struct {
		name             string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			registry := newRecordingRegistry()
			r := &HostReconciler{Client: c, Registry: registry}

			if _, err := r.Reconcile(t.Context(), ctrl.Request{NamespacedName: key}); err != nil {
				t.Fatalf("Reconcile() = %v", err)
			}

			_, registered := registry.hosts[key]
			if tc.wantRegistered && !registered {
				t.Error("expected RegisterHost to be called, got none")
			}
			if !tc.wantRegistered && registered {
				t.Errorf("did not expect RegisterHost, got %v", registry.hosts)
			}

			// wantDeregistered is only meaningful when the host was previously
			// registered; simulate that by pre-populating the registry and
			// checking the entry is (or isn't) dropped.
			if tc.wantDeregistered {
				pre := newRecordingRegistry()
				pre.hosts[key] = host.HostID
				r2 := &HostReconciler{Client: c, Registry: pre}
				if _, err := r2.Reconcile(t.Context(), ctrl.Request{NamespacedName: key}); err != nil {
					t.Fatalf("Reconcile() = %v", err)
				}
				if _, ok := pre.hosts[key]; ok {
					t.Error("expected DeregisterHost to be called, host is still registered")
				}
			}
		})
	}
